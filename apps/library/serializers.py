...
class MemberCreateSerializer(serializers.ModelSerializer):
    ...
    staff_number = serializers.CharField(required=False, allow_blank=True, write_only=True)
    registration_number = serializers.CharField(required=False, allow_blank=True, write_only=True)
    
    class Meta:
        model = Member
        fields = ['id', 'user', 'role', 'date_joined', 'active', 'staff_number', 'registration_number']
...
class BorrowTransactionUpdateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if status == 'Returned' and not return_date and self.instance.status != 'Returned':
...
class FineUpdateSerializer(serializers.ModelSerializer):
    ...
    def validate_paid(self, value):
        ...
        if self.instance and self.instance.paid and not value:
            raise serializers.ValidationError(
                "Cannot mark a paid fine as unpaid."
            )
        return value
...
class LibraryFinePaymentSerializer(serializers.ModelSerializer):
    class Meta:
        model = LibraryFinePayment
        fields = ["id", "member", "fine", "amount", "paid", "payment_date", "payment_method", "payment_reference"]
        depth = 1  
