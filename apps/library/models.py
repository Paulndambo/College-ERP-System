...
class Fine(AbsoluteBaseModel):
    ...
    fine_per_day = models.DecimalField(max_digits=5, decimal_places=2, default=0.50)  # Default $0.50/day
    ...
    def save(self, *args, **kwargs):
        if not self.borrow_transaction.return_date:
            ...
            overdue_days = (self.borrow_transaction.return_date - self.borrow_transaction.due_date).days
        ...
        if overdue_days > 0:
            if self.borrow_transaction.status == "Returned":
                ...
                self.calculated_fine = (overdue_days * self.fine_per_day) + self.borrow_transaction.book.unit_price
        else:
