    ...
    def validate(self, attrs):
        ...
        if room.students_assigned > 0:
            raise serializers.ValidationError(
                "Cannot delete room with students assigned."
            )

        if room.booking_set.filter(status="Checked In").exists():
            raise serializers.ValidationError(
                "Cannot delete room with bookings of status currently checked in."
            )
...
class CreateAndUpdateBookingSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Booking
        fields = ["hostel_room", "student", "status"]
        extra_kwargs = {
    ...
    def create(self, validated_data):
        ...
        if Booking.objects.filter(student=student).exists():
            raise serializers.ValidationError(
                "Student is already assigned to a hostel room"
            )
        ...
        with transaction.atomic():
            ...
            print(
                f"Booking created successfully. New students_assigned: {hostel_room.students_assigned}"
            )
    ...
    def validate(self, attrs):
        ...
        if new_status == "Confirmed" and instance:
            if instance.hostel_room.fully_occupied():
                raise serializers.ValidationError(
                    "Cannot confirm booking: Room is fully occupied"
                )
...
class RemoveStudentFromRoomSerializer(serializers.Serializer):
    ...
    def remove_student(self):
        ...
        with transaction.atomic():
            hostel_room = HostelRoom.objects.select_for_update().get(
                pk=student.hostel_room.id
            )
...
class UpdateStudentRoomSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        if student.hostel_room is None:
            raise serializers.ValidationError(
                "Student is not currently assigned to any room"
            )
    ...
    def create(self, validated_data):
        ...
        with transaction.atomic():
            ...
            old_room = HostelRoom.objects.select_for_update().get(
                pk=student.hostel_room.id
            )
