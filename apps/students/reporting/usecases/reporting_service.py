...
class SemesterReportingService:
    ...
    def create_student_invoice(student, semester):
        ...
        try:
            ...
            print(
                f"Invoice creation for student {student.id}: {'Success' if success else 'Failed'}"
            )
            return success
    ...
    def create_invoices_for_students(students, semester):
        ...
        for student in students:
            try:
                success = SemesterReportingService.create_student_invoice(
                    student, semester
                )
                results.append(
                    {
    ...
    def validate_semester_for_reporting(semester):
        ...
        if semester.status == "Closed":
            return {"is_valid": False, "error": "Cannot report to a closed semester."}
    ...
    def get_invoice_summary(invoice_results):
        ...
        summary = {
            ...
            "success_rate": (
                (successful_invoices / len(invoice_results)) * 100
                if invoice_results
                else 0
            ),
        }
    ...
    def handle_cohort_promotion(cohort, next_semester):
        ...
        try:
            ...
            promoted_reporting = SemesterReporting.objects.filter(
                cohort=cohort, semester=next_semester
