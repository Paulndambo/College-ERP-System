...
class ExamDataFilterSet(django_filters.FilterSet):
    ...
    reg_no = django_filters.CharFilter(
        field_name="student__registration_number", lookup_expr="icontains"
    )
    course_name = django_filters.CharFilter(
        field_name="course__name", lookup_expr="icontains"
    )
    semester_name = django_filters.CharFilter(
        field_name="semester__name", lookup_expr="icontains"
    )
    ...
    class Meta:
        model = ExamData
        fields = ["student", "semester", "cohort", "course", "reg_no"]
