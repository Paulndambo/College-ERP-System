...
from .models import Intake, StudentApplication, ApplicationDocument, ApplicationEducationHistory
...
class ApplicationDocumentCreateSerializer(serializers.ModelSerializer):
    student_application = serializers.PrimaryKeyRelatedField(queryset=StudentApplication.objects.all(), required=False)
    class Meta:
...
class StudentApplicationCreateSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        extra_kwargs = {
            ...
            'application_number': {'required': False, },
...
class StudentApplicationListDetailSerializer(serializers.ModelSerializer):
    ...
    def get_application_education_history(self, obj):
        ...
        education_history = ApplicationEducationHistory.objects.filter(student_application=obj)
        return ApplicationEducationHistoryListDetailSerializer(education_history, many=True).data
    ...
    def validate(self, data):
        ...
        try:
            ...
            if application.status == "Enrolled":
                raise serializers.ValidationError("This application has already been enrolled")
                
            
            if application.status != "Accepted":
                raise serializers.ValidationError("Only Accepted applications can be enrolled")
