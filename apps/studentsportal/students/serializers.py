...
from apps.students.models import MealCard, StudentAttendance, SemesterReporting
...
class StudentAttendanceSerializer(serializers.ModelSerializer):
    ...
    def get_course_code(self, obj):
        return (
            obj.session.course.course_code
            if obj.session and obj.session.course
            else None
        )

    def get_semester(self, obj):
        return (
            obj.session.semester.name if obj.session and obj.session.semester else None
        )

    def get_academic_year(self, obj):
        return (
            obj.session.cohort.get_academic_year()
            if obj.session and obj.session.cohort
            else None
        )
