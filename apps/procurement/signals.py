...
from apps.procurement.models import GoodsReceived, TenderAward, VendorPayment, VendorPaymentStatement
...
def create_vendor_payment_journal_entry(sender, instance, created, **kwargs):
    ...
    try:
        ...
        create_journal_entry(
            ...
            transactions=[
                {"account": expense_account, "amount": instance.amount, "is_debit": True},
        ...
        logger.error(f"Failed to create journal entry for vendor payment {instance.id}: {e}")
...
def create_initial_vendor_statement(sender, instance, created, **kwargs):
    ...
    try:
        ...
        logger.error(f"Failed to create initial statement for tender award {instance.id}: {e}")
