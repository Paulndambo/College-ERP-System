...
class VendorListSerializer(serializers.ModelSerializer):
    awards = TenderAwardNestedSerializer(
        many=True, read_only=True, source="awarded_tenders"
    )
...
class TenderApplicationCreateUpdateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if TenderApplication.objects.filter(
            tender=tender, company_name__iexact=company_name
        ).exists():
            raise serializers.ValidationError(
                "This company has already applied for this tender."
            )
