...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for title in tender_titles:
            ...
            start_date = timezone.now().date() + timedelta(days=random.randint(7, 14))
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"{'Created' if created else 'Updated'} Tender: {title}"
                )
            )
            ...
            for _ in range(random.randint(3, 5)):
                ...
                status = random.choices(
                    ["approved", "pending", "rejected"], weights=[0.4, 0.4, 0.2], k=1
                )[0]
                ...
                if (
                    status == "approved"
                    and not TenderAward.objects.filter(tender=tender).exists()
                ):
                    vendor_status = random.choice(["active", "inactive"])
                    vendor, _ = Vendor.objects.update_or_create(
                        ...
                        defaults={
                            ...
                            "company_registration_number": fake.bothify(
                                text="CRN-#####"
                            ),
                            "tax_pin": fake.bothify(text="KRA#######"),
                            "business_type": random.choice(
                                ["individual", "sole_proprietor", "limited", "ngo"]
                            ),
                            "status": vendor_status,
                        },
                    )
                    if not vendor.vendor_no:
                        vendor.save()
                    award_amount = random.randint(
                        int(projected_amount * 0.9), int(projected_amount * 1.1)
                    )

                    TenderAward.objects.update_or_create(
                        tender=tender,
                        defaults={
                            "vendor": vendor,
                    ...
                    tender.actual_amount = award_amount
                    ...
                    if po.items.count() == 0:
                        for _ in range(random.randint(1, 4)):
                            ...
                            category = (
                                Category.objects.filter(name=matched_cat).first()
                                if matched_cat
                                else None
                            )
